...
class IMDBCrawler(scrapy.Spider):
    ...
    def parseAMovie(self, response):
        ...
        data['Budget'] = response.xpath(
            '//button[text()="Budget"]/..//label[@class="ipc-metadata-list-item__list-content-item"]/text()').re(
            '[$€]{1}[0-9,]*')
        ...
        # data['Genre'] = response.xpath('//button[text()="Genres"]/..//li[@class="ipc-inline-list__item"]/a/text()').getall()
        data['Genre'] = response.xpath('//a/span[@class="ipc-chip__text"]/text()').getall()
        all_data = response.xpath('//script[@type="application/ld+json"]/text()').get()
        keywords = re.findall('keywords\":\"(.+?)"', all_data)
